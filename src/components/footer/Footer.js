      ...
      >
        ...
        <div className="flex-1 space-y-2">
          <div className="flex justify-between">
            <p>{data?.isPlaying ? <RiPauseCircleFill className="text-white" size={24} /> : ""}</p>
            <p>{data?.isPlaying ? <SiSpotify size={20} color={"#1ED760"} /> : ""}</p>
          </div>
          <div>
            <p className="font-bold text-white">{data?.isPlaying ? data.title : "Not Listening"}</p>
            <p className="text-xs text-gray-500 mt-2">{data?.isPlaying ? data.artist : "Spotify"}</p>
          </div>
        </div>
      </a>
